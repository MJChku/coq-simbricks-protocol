...
Lemma sorted_split :
  forall l1 l2,
    sorted_ts (l1 ++ l2) ->
    (forall x y, In x l1 -> In y l2 -> get_ts x <= get_ts y).
Proof.
  intros.
  induction l1.
  - inversion H0.
  - rewrite <- app_comm_cons in H.
    ...
    + subst.
      ...
      eapply or_intror in H1.
      rewrite <- in_app_iff with (l := (evx, tsx) :: l1) in H1.
      eapply sorted_ts_in_order.
      * apply H.
      * apply H1.
...
Proof.
  ...
  - destruct a as [ eva tsa ].
    ...
    + destruct l2.
      ...
      * destruct t as [ evt tst ].
        split.
        -- specialize H1 with (eva, tsa) (evt, tst).
           simpl in H1.
           apply H1.
           ++ left; reflexivity.
           ++ left; reflexivity.
...
Proof.
  ...
  - destruct x as [ evx tsx ].
    ...
    + subst.
      ...
      eapply sorted_ts_in_order.
      * apply H.
      * apply in_cons. apply in_or_app.
        right. apply H1.
    + apply IHl1.
      2: apply H0.
      rewrite <- app_comm_cons in H.
      remember (l1 ++ l2) as l. destruct l.
      * firstorder.
      * destruct a.
        destruct t.
        apply H.
...
Lemma process_event_sorted_trace_and_queue :
  forall t my_ts delay h tr q res h' tr' q',
    process_event t my_ts delay (h, (tr, q)) = (res, (h', (tr', q'))) ->
    sorted_ts (tr ++ q) ->
    sorted_ts (tr ++ [t]) ->
...
Proof.
  intros.
  destruct t as [ [ id ev ] ts ].
  apply sorted_from_split.
  - apply process_event_unchanged_trace in H.
    subst.
    apply sorted_ts_dist in H0.
    apply H0.
  - apply process_event_queue_remains_sorted in H.
    + apply H.
    + apply sorted_ts_dist in H0.
      apply H0.
  - intros.
    pose proof H.
    apply process_event_unchanged_trace in H5.
    subst.
    unfold process_event in H.
    unfold enq_event_ord in H.
    destruct ev; simpl in H; inversion H.
    5: {
      eapply insert_ts_elements in H7.
      2: apply H4.
      destruct H7.
      - subst.
        eapply sorted_split in H1.
        2: apply H3.
        2: apply in_eq.
        apply H1.
      - eapply sorted_split in H0.
        2: apply H3.
        2: apply H2.
        apply H0.
    }
...
Proof.
  ...
  induction q; intros.
  - inversion H. exists []. apply eq_sym. apply app_nil_r.
  - simpl in H.
...
(* commit_q preserves sorted property of trace ++ queue *)
Lemma commit_q_sorted_trace_and_queue :
  forall q ts tr tr' q',
    commit_q q ts tr = (tr', q') ->
    sorted_ts (tr ++ q) ->
    sorted_ts (tr' ++ q').
...
Proof.
  ...
  - destruct t as [ ev ts_ev ].
    simpl in H.
    destruct (ts_ev <=? ts).
    + eapply commit_q_trace_remains in H.
      2: { apply in_or_app. right. apply in_eq. }
      rewrite H0 in H.
      apply in_nil in H.
      contradiction.
    + inversion H.
...
Fixpoint no_future_events ( tr : list TimedEvent ) ( ts : nat ) : Prop :=
  match tr with
  | [] => True
  | (ev, ev_ts) :: tr' => ev_ts <= ts /\ (no_future_events tr' ts)
  end.

Lemma no_future_events_from_last :
  forall l ev ts,
    sorted_ts (l ++ [(ev, ts)]) -> no_future_events l ts.
Proof.
  intros.
  induction l.
  - firstorder.
  - destruct a as [ eva tsa ].
    split.
    + rewrite <- app_comm_cons in H.
      eapply sorted_ts_in_order with (q := (l ++ [(ev, ts)])).
      2: { apply in_cons. apply in_or_app. right. apply in_eq. }
      apply H.
    + apply IHl.
      rewrite <- app_comm_cons in H.
      destruct (l ++ [(ev, ts)]).
...
Lemma no_future_events_from_split :
  forall l1 l2 ts,
    no_future_events l1 ts ->
    no_future_events l2 ts ->
    no_future_events (l1 ++ l2) ts.
Proof.
  intros.
  induction l1.
  - rewrite app_nil_l.
...
Lemma no_future_events_bound :
  forall l ts,
    no_future_events l ts ->
    (forall x, In x l -> get_ts x <= ts).
Proof.
  intros.
  induction l.
  - inversion H0.
...
Lemma commit_q_trace_no_future_events :
  forall q ts tr tr' q',
    commit_q q ts tr = (tr', q') ->
    no_future_events tr ts ->
    no_future_events tr' ts.
Proof.
  intros. generalize dependent tr.
  induction q; intros.
  ...
  - destruct a as [ ev ts_ev ].
    simpl in H.
    remember (ts_ev <=? ts) as cmp. destruct cmp.
    + eapply IHq.
      * apply H.
      * apply no_future_events_from_split.
        -- apply H0.
        -- apply eq_sym in Heqcmp.
           rewrite Nat.leb_le in Heqcmp.
           simpl.
           split; [ | firstorder ].
           apply Heqcmp.
    + inversion H; subst.
...
Lemma consume_loop_trace_prefix :
...
Proof.
  ...
  - destruct a as [ [ id type ] ts_ev ].
    ...
    apply eq_sym in H3.
    eapply commit_q_trace_prefix in H3.
    destruct H3 as ( l & H3 ).
    eapply IHts in H.
    destruct H as ( l' & H ).
    subst.
  ...
  - destruct a as [ eva tsa ].
    ...
    + subst. destruct tr.
      * left. intros.
        ...
        subst. lia.
      * destruct t as [ evt tst ].
        remember ( tsa <? tst ) as cmp. destruct cmp.
        -- right.
           exists (evt, tst).
           split; [ | split ].
           ++ apply eq_sym in Heqcmp.
              rewrite Nat.ltb_lt in Heqcmp.
              simpl.
              lia.
           ++ apply in_cons. apply in_eq.
           ++ apply H0.
        -- apply eq_sym in Heqcmp.
           rewrite Nat.ltb_ge in Heqcmp.
           destruct H.
           apply Nat.le_antisymm in Heqcmp.
           2: apply H.
           subst.
           eapply IHtr in H1.
           2: apply H0.
    ...
    + pose proof H1.
      ...
      * left.
        ...
        destruct H3.
        destruct x.
        -- subst. eapply sorted_ts_in_order.
           ++ apply H.
           ++ apply in_cons.              
              apply H2.
        -- apply H1.
...
Lemma last_event_ts_append_front :
  forall x tr,
    tr <> [] -> last_event_ts tr = last_event_ts (x :: tr).
Proof.
  intros.
  unfold last_event_ts.
...
Lemma last_event_ts_extend_front :
  forall l tr,
    tr <> [] -> last_event_ts tr = last_event_ts (l ++ tr).
Proof.
  intros.
  unfold last_event_ts.
  rewrite <- last_event_extend_front.
  2: apply H.
  reflexivity.
Qed.

Lemma last_event_trunc_front :
  forall l1 l2 x,
    last_event (l1 ++ x :: l2) = last_event (x :: l2).
Proof.
  intros.
  induction l1.
  - reflexivity.
...
Proof.
  ...
  - destruct a as [ [ id type ] ts_ev ].
    ...
    + inversion H.
      subst.
      remember (last_event tr') as e. destruct e.
      * right.
        exists (id, type, ts_ev), t.
        split; [ | split ].
        -- rewrite <- last_event_extend_front.
           2: { unfold "<>". intros. inversion H2. }
           reflexivity.
        -- reflexivity.
        -- destruct t as [ evt tst ].
           unfold get_ts.
           apply eq_sym in H5.
           apply commit_q_trace_no_future_events in H5.
           2: eapply no_future_events_from_last; apply H0.
           apply eq_sym in Heqe.
           apply last_event_in in Heqe.
           eapply no_future_events_bound in H5.
           2: apply Heqe.
           apply H5.
      * destruct trpe.
        -- left. split.
        ...
        -- right.
           apply eq_sym in H5.
           eapply commit_q_trace_remains in H5.
           2: apply in_eq.
           apply eq_sym in Heqe.
           apply last_event_nil in Heqe.
           rewrite Heqe in H5.
    ...
    + destruct t as [ evt tst ].
      ...
      2: {        
        apply sorted_from_split.
        - apply process_event_sorted_trace_and_queue in Heqretpe.
          2: apply H1.
          2: {
            assert ((@app EInst.TE trpe
         (@cons TimedEvent
            (@pair (prod nat Event) nat (@pair nat Event id type) ts_ev)
            (@cons TimedEvent (@pair (prod nat Event) nat evt tst) ts))) = trpe ++ [(id, type, ts_ev)] ++ (evt, tst) :: ts) by reflexivity.
            rewrite H2 in H0.
          ...
          2: reflexivity.
          apply eq_sym in H5.
          apply commit_q_sorted_trace_and_queue in H5.
          2: apply Heqretpe.
          apply sorted_ts_dist in H5.
          apply H5.
        - apply sorted_ts_dist in H0.
          apply H0.
        - intros.
         ...
         (@cons TimedEvent
            ...
            apply sorted_ts_dist in H0.
            destruct H0.
            apply no_future_events_from_last in H0.
            apply H0.
          }
          eapply no_future_events_bound in H5.
          2: apply H2.
          ...
          destruct H0.
          destruct y as [ evy tsy ].
          eapply sorted_ts_in_order in H4.
          2: apply in_cons; apply H3.
          simpl. lia.
      }          
      2: {
        apply eq_sym in H5.
      ...
      * left.
        apply eq_sym in H5.
        apply commit_q_nil in H5.
        2: apply H.
        split.
        -- apply H5.
        -- apply H.
      * right.
        destruct H as ( x & y & Hlex & Hley & Hcmp ).
        exists x, y.
        split; [ | split ].
        -- assert ((@app EInst.TE trpe
          (@cons TimedEvent
           ...
           rewrite <- last_event_extend_front.
           2: apply not_eq_sym; apply nil_cons.
           rewrite last_event_trunc_front in Hlex.
           apply Hlex.
        -- apply Hley.
        -- apply Hcmp.
Qed.

Lemma consume_loop_trace_remains_sorted :
  forall ts ptr ptr_last_sync delay link_delay h tr p res h' tr' p',
    consume_loop ts ptr ptr_last_sync delay link_delay (h, (tr, p)) = (res, (h', (tr', p'))) ->
    sorted_ts (tr ++ ts) ->
    sorted_ts (tr ++ p) ->
    ...
    + apply sorted_from_split.
      * apply process_event_sorted_trace_and_queue in Heqretpe.
        ...
        2: {
          assert ((@app EInst.TE trpe
         (@cons TimedEvent
            (@pair (prod nat Event) nat (@pair nat Event id type) ts_ev) ts)) = trpe ++ [(id, type, ts_ev)] ++ ts) by reflexivity.
          rewrite H2 in H0.
          rewrite app_assoc in H0.
          apply sorted_ts_dist in H0.
          apply H0.
...
Lemma bounded_trace_dist :
  forall l1 l2 link_delay,
    bounded_trace (l1 ++ l2) link_delay ->
    bounded_trace l1 link_delay /\ bounded_trace l2 link_delay.
Proof.
  intros.
  induction l1.
  - split.
...
Lemma consume_loop_all_sync_bounded :
  forall ts ptr ptr_last_sync delay link_delay h tr p res h' tr' p',
    ...
    sorted_ts (tr ++ ts) ->
    sorted_ts (tr ++ p) ->
    bounded_trace (tr ++ ts) link_delay ->
    (p = [] \/ exists evt tst p', p = (evt, tst) :: p' /\ bounded_trace ((evt, tst) :: ts) link_delay) ->
    bounded_trace tr' link_delay.
Proof.
  intros.
  generalize dependent p'. generalize dependent tr'. generalize dependent h'.
  generalize dependent p. generalize dependent tr. generalize dependent h.
  induction ts; intros.
  - inversion H. subst. rewrite app_nil_r in H3. apply H3.
  - destruct a as [ [ id type ] ts_ev ].
    simpl in H.
    unfold bind in H.
    ...
    2: {
      apply sorted_from_split.
      - apply eq_sym in H8.
        apply commit_q_sorted_trace_and_queue in H8.
        2: {
          simpl in H0.
          destruct H0.
          subst.
          apply sorted_from_split.
          - apply sorted_ts_dist in H2.
            apply H2.
          - apply process_event_queue_remains_sorted in Heqretpe.
            2: apply sorted_ts_dist in H2; apply H2.
            apply Heqretpe.
          - intros.
            inversion Heqretpe.
            eapply insert_ts_elements in H11.
            2: apply H6.
            destruct H11.
            + admit.
            + 
        }
        apply sorted_ts_dist in H8.
        apply H8.
      - assert ((@app EInst.TE trpe
         (@cons TimedEvent
            (@pair (prod nat Event) nat (@pair nat Event id type) ts_ev) ts)) = trpe ++ [(id, type, ts_ev)] ++ ts).
        reflexivity.
        rewrite H5 in H1.
        rewrite app_assoc in H1.
        apply sorted_ts_dist in H1.
      ...
      - intros.
        apply eq_sym in H8.
        apply commit_q_trace_no_future_events in H8.
        2: {
          assert ((@app EInst.TE trpe
         (@cons TimedEvent
            (@pair (prod nat Event) nat (@pair nat Event id type) ts_ev) ts)) = trpe ++ [(id, type, ts_ev)] ++ ts).
          ...
          apply sorted_ts_dist in H1.
          destruct H1.
          apply no_future_events_from_last in H1.
          apply H1.
        }
        eapply no_future_events_bound in H8.
        2: apply H5.
        apply sorted_ts_dist in H1.
        destruct H1.
        destruct y as [ evy tsy ].
        eapply sorted_ts_in_order in H7.
        2: apply in_cons; apply H6.
        simpl. lia.
    }
    2: {
      destruct H4.
      - unfold all_sync in H0. destruct H0.
        subst.                
        inversion Heqretpe.
        subst.
        simpl in H8.
        rewrite Nat.leb_refl in H8.
        inversion H8.
        subst.
        rewrite <- app_assoc.
        apply H3.
      - destruct H4 as ( evt & tst & pw' & H4 ).
        destruct H4. subst.
        destruct trpe.
        + admit.
        + eapply bounded_trace_more_events.
          6: apply H3.
          * unfold incl.
            intros.
            apply in_app_or in H4.
            destruct H4; [ | apply in_inv in H4; destruct H4 ].
            -- apply eq_sym in H8.
               eapply commit_q_trace_remains in H8.
               2: apply H4.
               apply in_or_app.
               left. apply H8.
            -- admit.
            -- apply in_or_app.
               right. apply H4.
          * apply H1.
          * (* Use from above *) admit.
          * apply eq_sym in H8.
            apply commit_q_trace_prefix in H8.
            destruct H8 as ( l & H8 ).
            subst.
            reflexivity.
          * destruct ts.
            -- admit.
            -- rewrite <- last_event_ts_extend_front.
               2: apply not_eq_sym; apply nil_cons.
               rewrite <- last_event_ts_extend_front.
               2: apply not_eq_sym; apply nil_cons.
               rewrite <- last_event_ts_append_front.
               2: apply not_eq_sym; apply nil_cons.
               reflexivity.
    }
    2: {
    ...
    2: {
      destruct pce.
      - left. reflexivity.
      - right. destruct t as ( evt & tst ).
        exists evt, tst, pce.
        split; [ reflexivity | ].
        apply eq_sym in H8.
        apply commit_q_no_more_to_commit in H8.
        2: {
          apply process_event_queue_remains_sorted in Heqretpe.
          2: apply sorted_ts_dist in H2; apply H2.
          apply Heqretpe.
        }
        unfold committed_to_time in H8.
        destruct evt.
        destruct H8.
        apply bounded_trace_dist in H3.
        destruct H3.
        destruct ts.
        + firstorder.
        + destruct t as ( evt' & tst' ).
...
Admitted.              
  ...
  intros.
  apply bounded_trace_in_sorted.
    + eapply consume_loop_trace_remains_sorted.
      * apply H.
      * apply H1.
      * apply H2.
    + intros.
      remember (last_event tr') as le. destruct le.
      * destruct t as [ evle tsle ].
        remember (get_ts x <? tsle) as cmp.
        destruct cmp.
        -- apply eq_sym in Heqcmp. rewrite Nat.ltb_lt in Heqcmp.
           right.
           
      * apply eq_sym in Heqle.
        apply last_event_nil in Heqle.
        rewrite Heqle in H8.
        apply in_nil in H8.
        contradiction.
     *)
    (* Idea: We want to show every event after the trace has a sync event to cover. *)
    
Lemma sorted_ts_filter :
  forall l f,
    sorted_ts l -> sorted_ts (filter f l).
Proof.
  Admitted.           
...
Lemma consume_loop_bounded :
  forall ts ptr ptr_last_sync delay link_delay h tr p res h' tr' p',
    ...
    sorted_ts (tr ++ ts) ->
    sorted_ts (tr ++ p) ->
    bounded_trace (tr ++ (sync_filter ts)) link_delay ->
    bounded_trace tr link_delay ->
...
Proof.
  ...
  eapply bounded_trace_more_events.
  - unfold incl. intros.
    eapply consume_loop_all_sync_in_trace.
    + apply H.
    + apply sorted_ts_dist in H1.
      apply H1.
    + 
  - pose proof H0.
    apply sorted_ts_dist in H0.
    destruct H0.
    eapply consume_loop_trace_remains_sorted in H.
    2: apply H5.
    2: apply H1.
    apply sorted_ts_filter.
    apply H.
  - eapply consume_loop_trace_remains_sorted in H.
    2: apply H0.
    2: apply H1.
  ...
  - admit.
  - admit.
  (* eapply consume_loop_all_sync_bounded. *)
